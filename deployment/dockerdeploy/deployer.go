...
func (d *Deployer) AddNode(ctx context.Context, clusterID string) (string, error) {
	...
	if len(clusterInfoEx.NodesEx) == 0 {
...
func (d *Deployer) RemoveCluster(ctx context.Context, clusterID string) error {
	...
	var nodesToRemove []*DockerNodeInfo
...
func (d *Deployer) getNode(ctx context.Context, clusterID, nodeID string) (*DockerNodeInfo, error) {
	nodes, err := d.controller.ListNodes(ctx)
	if err != nil {
		return nil, errors.Wrap(err, "failed to list nodes")
	}

	var foundNode *DockerNodeInfo
	for _, node := range nodes {
