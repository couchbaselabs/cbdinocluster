...
func (d *Deployer) newCluster(ctx context.Context, def *clusterdef.Cluster) (*clusterInfo, error) {
	...
	dockerNodes := make([]*DockerNodeInfo, 0)
	...
	if err != nil {
		return nil, errors.Wrap(err, "failed to find new cluster after deployment")
	}
	...
	if nginxContainerId != "" {
		...
		err = d.updateLoadBalancer(ctx, nginxContainerId, thisCluster.Nodes, useDinoCerts)
		if err != nil {
	...
	slices.SortFunc(dockerNodes, func(a, b *DockerNodeInfo) int {
	...
	d.logger.Debug("reordered setup order", zap.Any("nodes", dockerNodes))

	var setupNodeOpts []*clustercontrol.SetupNewClusterNodeOptions
	for nodeIdx, node := range dockerNodes {
...
func (d *Deployer) addRemoveNodes(
	...
	var deployedNodes []*DockerNodeInfo
	...
	if len(nodesToRemove) > 0 {
		...
		for _, node := range clusterInfo.Nodes {
			if node.IsLoadBalancerNode() {
				err = d.updateLoadBalancer(ctx, node.ContainerID, postRemovalNodes, clusterInfo.UsingDinoCerts)
				if err != nil {
					return nil, errors.Wrap(err, "failed to update load balancer")
				}
			}
	...
	if len(nodesToAdd) > 0 {
		...
		for _, node := range clusterInfo.Nodes {
			if node.IsLoadBalancerNode() {
				err := d.updateLoadBalancer(ctx, node.ContainerID, thisCluster.Nodes, clusterInfo.UsingDinoCerts)
				if err != nil {
					return nil, errors.Wrap(err, "failed to update load balancer")
				}
			}
...
func (d *Deployer) removeNodes(ctx context.Context, nodes []*DockerNodeInfo) error {
