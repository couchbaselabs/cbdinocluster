...
type GetDeploymentOptionsResponse struct {
	...
	// plans
	// projects
	ServerVersions GetDeploymentOptionsResponse_ServerVersions `json:"serverVersions"`
	SuggestedCidr  string                                      `json:"suggestedCidr"`
}

type GetDeploymentOptionsResponse_ServerVersions struct {
...
func (c *Controller) GetDeploymentOptions(
	...
	resp := &GetDeploymentOptionsResponse{}

	path := fmt.Sprintf("/v2/organizations/%s/clusters/deployment-options", tenantID)
	err := c.doBasicReq(ctx, false, "GET", path, nil, &resp)
	if err != nil {
		return nil, err
	}
